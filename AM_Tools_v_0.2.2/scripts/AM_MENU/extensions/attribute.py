...
import collections
import logging
logger = logging.getLogger("anim_picker |")
import pymel.core as pm
import maya.cmds as cmds
import pymel.core.datatypes as datatypes
from AM_MENU.core.python_utils import  string_types
...
def addAttribute(node,
    ...
    if node.hasAttr(longName):
        logger.error("Attribute already exists")
        return
...
def addEnumAttribute(node,
    ...
    if node.hasAttr(longName):
        logger.warning("Attribute '" + longName + "' already exists" )
        return
...
def setRotOrder(node, s="XYZ"):
    ...
    if s not in a:
        logger.error("Invalid Rotorder : " + s )
        return False
...
def setInvertMirror(node, invList=None):
    ...
    for axis in invList:
        if axis not in aDic:
            logger.error ("Invalid Invert Axis : " + axis )
            return False
