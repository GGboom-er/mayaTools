        ...
        for side in self.adv_sides:
            for attr in lock_attr_list:
                ...
                if cmds.objExists(obj_attr):
                    cmds.setAttr(obj_attr, e=True, l=True, k=False, cb=False)
        # 第二步，转移控制面板属性到面部控制器上
        for side in self.adv_sides:
    ...
    def smile_pull_correct(self):
        ...
        for side in self.adv_sides:
            ...
            if cmds.objExists('ctrlMouth_M.zipLips%s' % side):
                utils.transfer_connect_attrs('ctrlMouth_M', smile_ctr, ['zipLips%s' % side])

    def add_teeth_ctrls(self):
        # 增加牙齿控制器
        def add_single_ctr(name, position):
            # 创建骨骼和所需组别
            ...
            control = controllerManager.Controller(controller_name, post='', color=[0.7, 0.7, 0.2], ge=True,
                                                   g=['Ext', 'Tra', 'Oft'], d='z')
            control.create(99)
        ...
        for part in ['upper', 'lower']:
            ...
            for key, position in positions.items():
                joint_grp, ctrl_grp = add_single_ctr('%sTeeth_%s' % (part, key), position)
