...
class AttrToolsUi(QtWidgets.QMainWindow):
    ...
    def bulkConnectFn(self):
        ...
        if mc.objExists(mainCtrlInfo):
            self.connectAttrUI.attrName.setText(currentItem)
    ...
    def getSelectCtrlFn(self):
        ...
        self.connectAttrUI.mainCtrl = self.MainCtrl
    ...
    def ctrlListWeightMenuB(self,pos):
        ...
        popMenu.addAction(QtWidgets.QAction(u'Del All Ctrl', self, triggered=self.Del_All_Ctrl))
        hitIndex = self.ctrlLW.indexAt(pos).column()
        if hitIndex == -1:
            popMenu.exec_(QtGui.QCursor.pos())
    ...
    def selMainCtrlFn(self):
        if mc.ls(sl =1):
            ...
            if self.attrToolsClass.selCtrl:
                ...
                self.addAttrInfolistItem()
                self.addCtrllistItem()
                self.connectAttrUI.attrName.setText('')
...
class BulkConnectUI(QtWidgets.QDialog):
    def __init__(self,parent=getMayaMainWindow()):
        ...
        self.completionList = ['visibility', 'lodVisibility', 'overrideEnabled','outputX','outputY','outputZ']
        restr = QtCore.QRegExp('[a-zA-Z]+$')
        self.re_validato = QtGui.QRegExpValidator(restr, self)
    ...
    def AHboxLayoutFn(self):
        ...
        self.attrName.setValidator(self.re_validato)
        self.attrName.setFocusPolicy(QtCore.Qt.ClickFocus)
        self.attrName.setCompleter(self.completer)
    ...
    def BHboxLayoutFn(self):
        ...
        self.driveAttrName.setValidator(self.re_validato)
    ...
    def connectAttrFn(self):
        mainAttr = self.attrName.text()
        ctrlList =  self.conCtrlName.text()
        attr = self.driveAttrName.text()
        if ctrlList and mainAttr and ctrlList == str(self.conCtrlNameList):
            for ctrl in self.conCtrlNameList:
                mc.connectAttr(self.mainCtrl+'.'+mainAttr,ctrl+'.'+attr,f =1)
